...
def explode_sum_rule_rows(df_rows: pd.DataFrame) -> pd.DataFrame:
    ...
    합산규칙(N)이 있는 경우:
      - 구분이 '...개/봉/통/팩' 기반이면 총 단위를 N묶음으로 분해
      - 구분이 비어있으면 '개'로 가정(합산규칙이 있을 때만)
    """
    out = []
    ...
    for _, r in df_rows.iterrows():
        ...
        # 적용 단위 판별
        if variant == "":
            unit_size, unit_label = 1, "개"  # 구분 없으면 1개로 간주(요청과 기존 동작 유지)
            is_bundle = True
        else:
...
# -----------------------------
if menu == "🧩 상품명 매칭 규칙":
    st.subheader("실제 상품명 → 표시될 상품명 + 합산규칙(개/봉/통/팩 묶음)")

    st.markdown(
        """
- **합산규칙(N)**: 해당 상품이 **개/봉/통/팩** 기반일 때, 총 단위를 **N묶음 + 나머지(<=N)묶음**으로 표현합니다.
  - 예: 합산규칙=5, 총 8개 → `5개 수량 1` + `3개 수량 1`
  - 예: 합산규칙=5, 총 8봉 → `5봉 수량 1` + `3봉 수량 1`
  - 예: 합산규칙=5, 총 12팩 → `5팩 수량 2` + `2팩 수량 1`
- g/kg/단/기타 단위에는 합산규칙이 적용되지 않습니다.
"""
    )
    ...
    st.info('예) 오렌지를 5묶음으로 표현하려면 `pattern=오렌지`, `display_name=오렌지`, `합산규칙=5`로 설정하세요. (개/봉/통/팩 모두 적용)')
